...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str,default='./modle_save/CNN_CTC_densenet.h5')
    # 最终结果输出路径
    parser.add_argument('--submission_path', type=str, default='./temp/answer.json')
    # 测试集图片路径
    parser.add_argument('--test_image_path', type=str, default='./dataset/test/images')
    # 测试集.json路径
    parser.add_argument('--test_null_path', type=str,default='./dataset/test/label/test_temp.json')
    opt = parser.parse_args()

    character_map_table = get_chinese_dict(opt.char_path)
    print(character_map_table)
    # 图片 shape
    ...
    # 测试集null_josn路径
    test_label_json_file = opt.test_null_path
    ...
    with open(test_label_json_file, 'r', encoding='utf-8') as in_file:
        ...
        for idx, info in enumerate(label_info_dict.items()):
            # "x.jpg":{"result":"伍拾元整","confidence":0.8587}
            ...
            # 图片预测label
            label_info_dict[image_name]['result'] = rec_result
            # 图片预测置信度
            label_info_dict[image_name]['confidence'] = 1
